...
let guessedNumber = sanitizeGuess(guess);
...
function showAlertMessage(alertMessage) {
  ...
  if (attempt !== 0) {
    alert(`${alertMessage} and you have ${attempt} attempts left.`);
    promptMessage = "Try again";
    guessedNumber = sanitizeGuess(prompt(promptMessage));
  }
}
...
function sanitizeGuess(guess) {
  ...
  while (isNaN(guessedNumber)) {
    promptMessage =
      "You entered an invalid number. Please enter a valid number";
